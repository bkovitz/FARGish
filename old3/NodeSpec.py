...
class BaseNodeSpec(ABC):
    ...
    def see_all(self, g, nodes=None):
        ...
        if nodes is None:
            nodes = g.nodeids() # All nodes: inefficient
    ...
    def see_one(self, g, nodes=None, multiplier=None):
        ...
        ns = WeightedNodes(g, self.see_all(g, nodes))
        return ns.choose1()
#        if ns:
...
class TupleCriterion(ABC):
    '''Specifies a criterion that a tuple of nodes, such as is generated by
...
class OLDBuildSpec(NiceRepr):
    ...
    def build(self, g, old_nodeid):
        ...
        if self.new_node_args:
            new_nodeid = g.add_node(self.new_nodeclass(*self.new_node_args))
        else:
            new_nodeid = g.add_node(self.new_nodeclass)
        for ls in as_iter(self.link_specs):
